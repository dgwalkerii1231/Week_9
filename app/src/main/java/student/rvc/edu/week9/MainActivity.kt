...
class MainActivity : AppCompatActivity() {
    ...
    fun ChangeImage(){
        ...
        //Tweak this code for imgMole to get past errors due to unresolved reference for ImageButton

        var btnImgButton = findViewById<ImageButton>(R.id.imgMole)
        y = ((Math.random () * getScreenHeight()) + 50) .toFloat ()
        x = ((Math.random () * getScreenWidth()) + 50) .toFloat ()
        btnImgButton.setTranslationX(x)
        btnImgButton.setTranslationY(y)
